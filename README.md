...
The agent acts as a **Guildmaster & Spirited Adventure Companion**:
1. **Story-Driven RPG Quests:** Every quest features an immersive adventure hook from in-world NPCs (inventors, merchants, scouts, scholars) so you're solving real dilemmas rather than dry exercises.
2. **Modern Version Verification:** Automatically checks the web for current stable releases and modern API paradigms (e.g. GSAP v3+, Anime.js v4+, Godot 4.x, Tailwind v4) to prevent teaching deprecated syntax.
3. **Calibration:** Starts with a single practical task to gauge starting level (no long placement tests).
4. **Just-In-Time Spawning:** Quests are generated one-by-one only after the prior quest is validated.
5. **Observable Evidence:** Progression requires actual output, command logs, or code execution (zero theoretical essay questions).
6. **Anti-Spoiler Hints:** Quests contain zero hints; dynamic hints are unlocked one tier at a time on explicit request.
7. **Stateful Vault & Campaign:** Tracks active progress, unlocked spells/concepts, and your ongoing story chronicle in `campaign.md`.
...
├── STORY.md            # narrative RPG framework & worldbuilding engine
...
└── learning/
    └── <topic>/
        ...
        ├── quests/             # spoiler-free quest files with story hooks
        │   ├── 001-<slug>.md
        │   ├── 002-<slug>.md
        │   └── ...
